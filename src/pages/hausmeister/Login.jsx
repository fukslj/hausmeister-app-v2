...
export default function HausmeisterLogin() {
  ...
  function handleInput(index, e) {
    ...
    } else if (neu.every(d => d)) {
      anmeldenMitPin(neu.join(''))
    }
  }
  ...
  function pressNum(v) {
    if (v === 'del') {
      ...
      if (letzterIndex >= 0) {
        const neu = [...pin]; neu[letzterIndex] = ''; setPin(neu)
      }
      return
    }
  ...
  return (
    <div style={{ fontFamily: 'var(--font)', minHeight: '100vh', display: 'flex' }}>
      {/* Links */}
      <div style={{ flex: 1, background: 'var(--hm-bg)', padding: '48px 40px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <div>
      ...
      <div style={{ flex: 1, background: 'white', padding: '48px 40px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginBottom: 28 }}>
          {pin.map((digit, i) => (
            <input
              ...
              style={{
                width: 56, height: 64, borderRadius: 10, textAlign: 'center', fontSize: 28, fontWeight: 500,
                fontFamily: 'var(--font)', background: '#F8F7F2', color: '#04342C',
                border: digit ? '1.5px solid #1D9E75' : '0.5px solid #D3D1C7',
        ...
        {istTouch && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, maxWidth: 220, margin: '0 auto 20px' }}>
            {['1','2','3','4','5','6','7','8','9','','0','del'].map((k, i) => (
              k === '' ? <div key={i} /> :
              <button key={i} onClick={() => pressNum(k)} style={{
                height: 48, borderRadius: 10, background: '#F8F7F2', border: '0.5px solid #D3D1C7',
                fontSize: k === 'del' ? 14 : 18, fontWeight: 500, color: k === 'del' ? '#888780' : '#1A1A18',
                fontFamily: 'var(--font)',
              }}>{k === 'del' ? '⌫' : k}</button>
            ))}
          </div>
        ...
        {!istTouch && <div style={{ fontSize: 12, color: '#888780', textAlign: 'center', marginBottom: 20 }}>Tastatur verwenden — springt automatisch weiter</div>}

        {fehler && <div style={{ fontSize: 12, color: '#C0392B', padding: '8px 12px', background: '#FDECEB', borderRadius: 8, marginBottom: 12 }}>{fehler}</div>}

        <button
          disabled={!pinVoll || laden}
          onClick={() => anmeldenMitPin(pin.join(''))}
